...
1. **[architecture.mmd](diagrams/architecture.mmd)**
   - NQL 처리 파이프라인
   - 각 단계별 색상 구분
   - GitHub에서 직접 렌더링

2. **[data-pipeline.mmd](diagrams/data-pipeline.mmd)**
   - 데이터 수집부터 저장소까지
   - Producer/Consumer 구조

3. **[rrf-scoring.mmd](diagrams/rrf-scoring.mmd)**
   - RRF 스코어링 과정
   - BM25 + Vector 통합
...
- GitHub에서 자동 렌더링
- 마크다운 파일로 버전 관리 가능
- [mermaid.live](https://mermaid.live)에서 편집 가능
